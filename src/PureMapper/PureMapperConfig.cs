namespace Kritikos.PureMapper
{
	...
	public class PureMapperConfig : IPureMapperConfig
	{
		...
		public IPureMapperConfig Map<TSource, TDestination>(
			Func<IPureMapperResolver, Expression<Func<TSource, TDestination>>> map) where TSource : class 
																					where TDestination : class
		{
			Func<IPureMapperResolver, Expression<Func<TSource, TDestination>>> f = resolver => x => (x == null) ? null : map(resolver).Invoke(x);
